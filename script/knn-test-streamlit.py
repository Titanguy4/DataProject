...
# Streamlit interface
st.title("K-Plus Proches Voisins (KNN) pour la Réduction d'Échelle des Données")
st.write("Ce projet vise à développer une stratégie pour réduire l'échelle des données de hauteur et de vitesse de l'eau, en passant d'une résolution grossière à une résolution fine.")
st.write("L'objectif est de prédire des **indicateurs de danger à haute résolution** pour les inondations urbaines, basés sur des prédictions à basse résolution. Le modèle KNN est utilisé pour établir la correspondance entre ces deux types de données.")
